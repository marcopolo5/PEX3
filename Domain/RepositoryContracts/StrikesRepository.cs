...
namespace Domain.RepositoryContracts
{
    public class StrikesRepository : GenericRepository<Strikes>
    {
        ...
        {
            string sqlGetStrikes = @"select * from Strikes where userid=@Id";
        ...
        {
            string sqlGetUserReport = @"select * from UserReports where (userid=@Userid and reporteduserid=@Reporteduserid";
            using (var connection = CreateConnection())
            {
